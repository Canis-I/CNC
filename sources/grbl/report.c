...
void report_gcode_modes()
{
  ...
  if (gc_state.modal.coolant) { // Note: Multiple coolant states may be active at the same time.
    if (gc_state.modal.coolant & PL_COND_FLAG_DRILL_MIST) { serial_write('7'); }
    if (gc_state.modal.coolant & PL_COND_FLAG_DRILL_FLOOD) { serial_write('8'); }
  } else { serial_write('9'); }
...
void report_realtime_status()
{
  ...
  // Report realtime feed speed
    ...
    if (sys.report_ovr_counter > 0) { sys.report_ovr_counter--; }
      ...
      uint8_t cl_state = get_drill_state();
      if (sp_state || cl_state) {
        ...
        if (cl_state & DRILL_STATE_FLOOD) { serial_write('F'); }
        if (cl_state & DRILL_STATE_MIST) { serial_write('M'); }
      }  
    }
  #endif
