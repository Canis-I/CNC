    ...
    if (rt_exec & (EXEC_COOLANT_FLOOD_OVR_TOGGLE | EXEC_COOLANT_MIST_OVR_TOGGLE)) {
      if ((sys.state == STATE_IDLE) || (sys.state & (STATE_CYCLE | STATE_HOLD | STATE_JOG))) {
        ...
        if (rt_exec & EXEC_COOLANT_MIST_OVR_TOGGLE) {
          if (coolant_state & DRILL_MIST_ENABLE) { bit_false(coolant_state, DRILL_MIST_ENABLE); }
          else { coolant_state |= DRILL_MIST_ENABLE; }
        }
        if (rt_exec & EXEC_COOLANT_FLOOD_OVR_TOGGLE) {
          if (coolant_state & DRILL_FLOOD_ENABLE) { bit_false(coolant_state, DRILL_FLOOD_ENABLE); }
          else { coolant_state |= DRILL_FLOOD_ENABLE; }
        }
          set_drill_state(coolant_state); // Report counter set in set_drill_state().
...
static void protocol_exec_rt_suspend()
{
  ...
  if (block == NULL) {
    ...
    restore_condition = (block->condition & PL_COND_SPINDLE_MASK) | get_drill_state();
      ...
      if (sys.state & (STATE_SAFETY_DOOR | STATE_SLEEP)) {
        ...
        if (bit_isfalse(sys.suspend,SUSPEND_RETRACT_COMPLETE)) {
          ...
          sys.spindle_stop_ovr = SPINDLE_STOP_OVR_DISABLED;
            ...
            set_drill_state(DRILL_DISABLE);     // De-energize
            ...
            if ((bit_istrue(settings.flags,BITFLAG_HOMING_ENABLE)) &&
              ...
              spindle_set_state(SPINDLE_DISABLE,0.0); // De-energize
              coolant_set_state(DRILL_DISABLE); // De-energize
          ...
          if (sys.suspend & SUSPEND_INITIATE_RESTORE) {
            ...
            if (gc_state.modal.coolant != DRILL_DISABLE) {
              // Block if safety door re-opened during prior restore actions.
              if (bit_isfalse(sys.suspend,SUSPEND_RESTART_RETRACT)) {
                // NOTE: Laser mode will honor this delay. An exhaust system is often controlled by this pin.
                  set_drill_state((restore_condition & (PL_COND_FLAG_DRILL_FLOOD | PL_COND_FLAG_DRILL_MIST)));
